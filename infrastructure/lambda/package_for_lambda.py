...
1. Creating two separate zip files for Lambda deployment:
   - app.zip: Contains the Lambda function code from the 'src' directory
...
e.g. 
pip install -r requirements.txt --python-version 3.12 --platform manylinux2014_aarch64 \
    --target ./packaging/_dependencies --only-binary=:all:
...
def create_lambda_package():
    ...
    # get the path to the lambda folder
